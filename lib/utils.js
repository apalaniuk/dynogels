...
utils.estimateDynamoAttributeSize = function(item) {
  switch (typeof item) {
    case 'undefined': return 0;
    case 'boolean': return 4;
    case 'number': return 2 * item.toString().length; // https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Limits.html#limits-data-types-numbers
    case 'string': return 2 * item.length;
    case 'object': return !item ? 0 : Object
      .keys(item)
      .reduce((total, key) => sizeOf(key) + sizeOf(item[key]) + total, 0); // Overestimates array size (I think). Meh.
  };
}
