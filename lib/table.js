  ...
  internals.callHooks(self, '_before', 'update', (callback) => {
    ...
    self.sendRequest('update', params, (err, data) => {
      ...
      internals.callHooks(self, '_after', 'update', (callback) => {
        callback(null, self.initItem(self.serializer.deserializeItem(data.Attributes || params.Key)));
      }, callback);
    });
  });
};
...
Table.prototype.transaction = {
  create: function (item, params) {
    return {
      item,
      table: this,
      callHooks: (stage, startFun, callback) => {
        internals.callHooks(this, stage, 'create', startFun, callback);
      },
      buildData: () => {
        const data = this.schema.applyDefaults(item);

        const paramName = _.isString(this.schema.createdAt)
          ? this.schema.createdAt
          : 'createdAt';

        if (this.schema.timestamps && this.schema.createdAt !== false && !_.has(data, paramName)) {
          data[paramName] = new Date().toISOString();
        }
  ...
  modify: function (item, params) {
    return {
      item,
      table: this,
      callHooks: (stage, startFun, callback) => {
        internals.callHooks(this, stage, 'update', startFun, callback);
      },
      buildData: () => {
        const schemaValidation = internals.validateItemFragment(item, this.schema);
        if (schemaValidation.error) {
          throw _.assign(new Error(`Schema validation error while creating update transaction item for table ${this.tableName()}: ${JSON.stringify(schemaValidation.error)}`), {
            name: 'DynogelsTransactionModifyError',
            detail: schemaValidation.error
          });
        }
  ...
  destroy: function (item, params) {
    return {
      item,
      table: this,
      callHooks: (stage, startFun, callback) => {
        internals.callHooks(this, stage, 'destroy', startFun, callback);
      },
      buildData: () => {
        const schemaValidation = internals.validateItemFragment(item, this.schema);
        if (schemaValidation.error) {
          throw _.assign(new Error(`Schema validation error while creating destroy transaction item for table ${this.tableName()}: ${JSON.stringify(schemaValidation.error)}`), {
            name: 'DynogelsTransactionDestroyError',
            detail: schemaValidation.error
          });
        }
...
Table.prototype.dynamoCreateTableParams = function (options) {
  const self = this;
  ...
  if (Object.prototype.hasOwnProperty.call(options, 'streamSpecification') && typeof options.streamSpecification === 'object') {
...
Table.prototype.createTable = function (options, callback) {
  ...
  const params = self.dynamoCreateTableParams(options);
